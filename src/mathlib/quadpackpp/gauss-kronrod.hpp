 ...
 *
 * Copyright (C) 2015 Samuel Leweke
...
namespace QuadPack
{
	...
	private:
		...
		Real legendre_err(size_t deg, Real x, Real& err);
		Real legendre_deriv(size_t deg, Real x);	
	 ...
	 */
	template <class Real>
	Real GaussKronrod<Real>::legendre_err(size_t n, Real x, Real& err)
	{
		if (n == 0) {
			err = Real(0);
			return Real(1);
	 ...
	 */
	template <class Real>
	Real GaussKronrod<Real>::legendre_deriv(size_t n, Real x)
	{
		if (n == 0)   
			return Real(0);
		else if (n == 1)
	...
	*/
														 ...
														 const Real result_asc) const
	{
		...
		if (result_asc != Real(0) && err != Real(0))
		{
			...
			const Real exponent = Real(3)/Real(2);
			const Real scale = pow((200 * err / result_asc), exponent);
		...
		{
	      const Real min_err = 50 * this->eps_ * result_abs ;
	...
	template <class Real> template <class Func>
	void GaussKronrod<Real>::qk(Func& f, const Real& a, const Real& b,
										 Real& result, Real& abserr, 
										 Real& resabs, Real& resasc) const
	{
		...
		for (size_t j = 0; j < (n_ - 1) / 2; j++)
		{
	      size_t jtw = j * 2 + 1;        /* j=1,2,3 jtw=2,4,6 */
	      const Real abscissa = half_length * xgk_[jtw];
		  const Real fval1 = f(center - abscissa);
		  const Real fval2 = f(center + abscissa);
	      const Real fsum = fval1 + fval2;
	      fv1[jtw] = fval1;
	      fv2[jtw] = fval2;
		...
		{
	      size_t jtwm1 = j * 2;
	      const Real abscissa = half_length * xgk_[jtwm1];
	 	  const Real fval1 = f(center - abscissa);
		  const Real fval2 = f(center + abscissa);
		  fv1[jtwm1] = fval1;
	      fv2[jtwm1] = fval2;
