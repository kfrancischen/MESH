...
namespace QuadPack
{
	...
	protected:
		...
		const Real inline abs(const Real& x) const { return (x < Real(0)) ? -(x) : x; }
		const Real inline max(const Real& a, const Real& b) const { return a > b ? a : b; }
		const Real inline min(const Real& a, const Real& b) const { return a < b ? a : b; }
		
	public:
		Machar();
		~Machar() {}
		
		const Real inline get_eps() const { return eps_; }
		const Real inline get_xmin() const { return xmin_; }
		const Real inline get_xmax() const { return xmax_; }
	};
	...
	{
		long int 
		ibeta, it, irnd, machep, negep, iexp, minexp, maxexp;
		Real epsneg;
		
		const Real one(1);
		const Real two = one + one;
		const Real zero(0);
